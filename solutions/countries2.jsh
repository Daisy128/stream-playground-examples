...
countries.stream().filter(country -> country.getName().toLowerCase().charAt(country.getName().length()-1) == country.getName().toLowerCase().charAt(0)).map(Country::getName).forEach(System.out::println);
...
countries.stream().sorted(Comparator.comparingInt(country -> country.getTimezones().size())).forEach(country -> country.getName()+":"+country.getPopulation());
...
countries.stream().filter(country -> country.getArea() != null).max(Comparator.comparingInt(country -> country.getArea().intValue())).get();
